...
class Client {
  ...
  getPlayer(args, shard = this.defaultShard) {
    if (typeof args !== 'object' || typeof shard !== 'string')
      throw new Error('Requires (object, !string)');

    if (args.id) {
      return this._baseRequest({
        endpoint: Array.isArray(args.id) ? 'players' : `players/${args.id}`,
        shard,
  ...
  getCurrentSeason(shard = this.defaultShard) {
    return this._baseRequest({ endpoint: 'seasons', shard })
      .then(
        seasons =>
          new Season(
            seasons.data.find(season => season.attributes.isCurrentSeason),
            this
          )
      )
      .catch(e => {
  ...
  async getManyPlayerSeason(args, season, shard) {
    if (typeof args !== 'object' || typeof season !== 'string')
      throw new Error('Requires (object, string, !string, !string)');

    if (!args.ids && !args.names && !args.players)
      throw new Error('Invalid use of <Client>.getManyPlayerSeason()');
    ...
    if (playersArray.length > 10)
      ...
      return Promise.all(
        playersArray.map(player => this.getPlayerSeason(player, season, shard))
      )
      ...
      await Util.asyncForEach(GAME_MODES, async mode => {
        gameModeStats[mode] = await this._baseRequest({
          endpoint: `seasons/${season}/gameMode/${mode}/players`,
          params: {
            'filter[playerIds]': playersArray.map(p => p.id).join()
          },
          shard: shard || this.defaultShard
        }).then(psArray => psArray.data);
      }).catch(e => {
        throw e;
      });
      ...
      return playersArray.map(player => {
        ...
        GAME_MODES.map(mode => {
          _.merge(
            playerSeason,
            gameModeStats[mode].find(
              ps => ps.relationships.player.data.id === player.id
            )
          );
        });
  ...
  getMatch(id, shard = this.defaultShard) {
    if (typeof id !== 'string' || typeof shard !== 'string')
      throw new Error('Requires (string, !string)');
  ...
  getSamples(createdAt, shard = this.defaultShard) {
    return this._baseRequest({
      ...
      .then(samples =>
        samples.data.relationships.matches.data.map(m => new Match(m.id, this))
      )
      .catch(e => {
        throw e;
      });
    ...
    return axios
      ...
      .catch(e => {
        throw e.response
          ...
          : { status: 404, statusText: e.message };
      });
  }
